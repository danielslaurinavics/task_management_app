...
const companyMid = require('../middleware/companyAuth');
...
router.get('/login', auth.redirectIfLoggedIn, (req, res) => res.render('./users/login'));
router.post('/login', auth.redirectIfLoggedIn, userCon.login);
router.post('/logout', auth.authenticate, userCon.logout);

// Registration routes
router.get('/register', auth.redirectIfLoggedIn, (req, res) => res.render('./users/register'));
router.post('/register', auth.redirectIfLoggedIn, userCon.register);
